...
int main()
{
    // open reciver's read path
    char argv[2][10] = {"1.pipe", "2.pipe"};
    int fd_reci = open(argv[0], O_RDONLY);
    ERROR_CHECK(fd_reci, -1, "open");
    // open sender's write path
        ...
        if(select(fd_reci+1, &rdset, NULL, NULL, &timeout) == 0) {
            ...
            if(FD_ISSET(STDIN_FILENO, &rdset)){
                ...
                if(read(STDIN_FILENO, buf, sizeof(buf)-1) == 0){
                    strcpy(p, "Now quitting chat!\n");
                    sprintf(message, "Jack %04d-%02d-%02d %02d:%02d:%02d\nI'm leaving now!", 
                            local->tm_year+1900, 
                            local->tm_mon+1, 
                            local->tm_mday, 
                            local->tm_hour, 
            ...
            if(FD_ISSET(fd_reci, &rdset)){
                char message[1100] = {0};
                if(read(fd_reci, message, sizeof(message)-1) == 0){
                    strcpy(p, message);
                    break;
                }
                strcpy(p, message);
